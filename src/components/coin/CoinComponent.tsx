      ...
      <ComponentBlock>
        <div className="flex h-full w-full items-center justify-center perspective-near transform-3d">
          <div className="relative h-24 w-24">
            <div className="animate-rotate relative h-24 w-24 transform-3d">
              {/* The "Edge" of the coin */}
              <div className="absolute h-24 w-1.5 transform-[translateX(48px)_rotateY(90deg)] bg-[#ffaa04]" />

              {/* Front Outer Layer (Translates slightly forward) */}
              <div className="absolute inset-0 h-full w-full transform-[translateZ(3px)] rounded-full bg-[#ffcd6c] backface-hidden">
                <CoinIcon />
              </div>

              {/* Front Inner Layer (Translates backwards) */}
              <div className="absolute inset-0 h-full w-full transform-[rotateY(180deg)_translateZ(-3px)] rounded-full bg-[#ffaa04] backface-hidden" />

              {/* Back Outer Layer (Rotated 180 and translates forward to face the other way) */}
              <div className="absolute inset-0 h-full w-full transform-[rotateY(180deg)_translateZ(3px)] rounded-full bg-[#ffcd6c] backface-hidden">
                <CoinIcon />
              </div>

              {/* Back Inner Layer */}
              <div className="absolute inset-0 h-full w-full transform-[translateZ(-3px)] rounded-full bg-[#ffaa04] backface-hidden" />
            </div>
          </div>
        </div>
      </ComponentBlock>
