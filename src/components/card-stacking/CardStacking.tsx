    ...
    <div className="grid h-full w-full items-center justify-center">
      {new Array(LENGTH).fill(0).map((_, i) => (
        <div
          ...
          className="shadow-accent-foreground bg-card h-18 w-80 origin-bottom -translate-y-[calc(var(--index)*13%)] scale-[calc(1-var(--index)*0.05)] rounded-xl border border-neutral-200 shadow-xs [grid-area:1/1] dark:border-neutral-700"
          key={i}
        />
      ))}
    </div>
